...
class VectorModel:
    ...
    def _convert_to_vectors(self):
        tf_vector = []
        idf_vector = []
        tf_dict = tf.calculate(self.inverted_idx, self.docs, self.terms_in_doc)
        # print(tf_dict)
        ...
        self.doc_vectors = (np.array(tf_vector) *
                            np.array(idf_vector)[:, np.newaxis]).T
        for key in self.docs:
            self.idx_map.append(key)
        # print(self.idx_map)
    ...
    def retrieve(self, query):
        ...
        if (all_zero == True):
            ...
            result = [cosine(query_vector, doc_vector)
                      for doc_vector in self.doc_vectors]
        # result = list(filter(lambda a: a != 0, result))
        res_dict = list_to_dict(result)
        result = sorted_descending(res_dict)
        new_res = []
        for tup in result:
            # if (tup[1] != 0):
            new_tup = (self.idx_map[tup[0]], tup[1])
            new_res.append(new_tup)
        result = new_res
        return result
...
if __name__ == "__main__":
    docs = docs = {
        "909": "abc alo ola 456 zzz ola all",
        "1080": "alo ola 321 123 all",
        "772": "hello 123 456 123 all",
        "114": "hello alo ola abc 123 456 zzz all",
        "564": "123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 123 all"
    }
